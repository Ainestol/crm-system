<?php
/** @var array $user */
/** @var array{primary_count:int, sub_a:int, sub_b:int, sub_c:int} $stats */
/** @var array{label:string, href:string, count_label:string} $primary */
?>
<?php require dirname(__DIR__) . '/_partials/dashboard_styles.php'; ?>
<div class="rd-wrap">
    <?php require dirname(__DIR__) . '/_partials/dashboard_header.php'; ?>

    <div class="rd-primary-card">
        <div class="rd-primary-left">
            <div class="rd-primary-label">🎯 Co mám teď udělat?</div>
            <div class="rd-primary-headline">Tvoje obchodní leady</div>
            <div class="rd-primary-sub"><?= htmlspecialchars($primary['count_label'], ENT_QUOTES, 'UTF-8') ?></div>
        </div>
        <a href="<?= htmlspecialchars($primary['href'], ENT_QUOTES, 'UTF-8') ?>" class="rd-primary-btn">
            <?= htmlspecialchars($primary['label'], ENT_QUOTES, 'UTF-8') ?> →
        </a>
    </div>

    <div class="rd-stats">
        <div class="rd-stat">
            <div class="rd-stat-label">💼 Aktivní leady</div>
            <div class="rd-stat-value"><?= (int) $stats['primary_count'] ?></div>
            <div class="rd-stat-sub">v rozpracování</div>
        </div>
        <div class="rd-stat">
            <div class="rd-stat-label">📅 Schůzky týden</div>
            <div class="rd-stat-value"><?= (int) $stats['sub_a'] ?></div>
            <div class="rd-stat-sub">tento týden</div>
        </div>
        <div class="rd-stat">
            <div class="rd-stat-label">📤 Čeká BO</div>
            <div class="rd-stat-value"><?= (int) $stats['sub_b'] ?></div>
            <div class="rd-stat-sub">smluv k aktivaci</div>
        </div>
    </div>

    <div class="rd-quick-actions">
        <h3>⚡ Rychlé akce</h3>
        <div class="rd-quick-list">
            <a href="/oz/queue" class="rd-quick-btn">📋 Příchozí leady</a>
            <a href="/oz/leads" class="rd-quick-btn">💼 Pracovní plocha</a>
            <a href="/oz/search" class="rd-quick-btn">🔍 Vyhledat klienta</a>
            <a href="/oz/campaigns" class="rd-quick-btn">🎯 Moje kampaně</a>
            <a href="/oz/email-leads" class="rd-quick-btn">📧 Email leady</a>
            <a href="/oz" class="rd-quick-btn">📊 Můj měsíc</a>
            <a href="/oz/premium/new" class="rd-quick-btn">💎 Nová premium objednávka</a>
        </div>
    </div>
</div>
