...
<section class="card">
    ...
    <!-- ── Navolávačka (jen vlastní pohled) ── -->
    <?php if ($role === 'navolavacka') { ?>
    <div class="dash-cards">
        <a href="<?= crm_h(crm_url('/caller')) ?>" class="dash-card dash-card--primary">
            <span class="dash-card__icon">📞</span>
            <span class="dash-card__title">Provolávání</span>
            <span class="dash-card__desc">Moje kontakty &amp; volání</span>
        </a>
        <a href="<?= crm_h(crm_url('/caller/stats')) ?>" class="dash-card">
            <span class="dash-card__icon">📊</span>
            <span class="dash-card__title">Moje statistiky</span>
        </a>
        <a href="<?= crm_h(crm_url('/caller/calendar')) ?>" class="dash-card">
            <span class="dash-card__icon">📅</span>
            <span class="dash-card__title">Kalendář callbacků</span>
        </a>
    </div>
    <?php } ?>

    <!-- ── Čistička (jen vlastní pohled) ── -->
    <?php if ($role === 'cisticka') { ?>
    <div class="dash-cards">
        <a href="<?= crm_h(crm_url('/cisticka')) ?>" class="dash-card dash-card--primary">
            <span class="dash-card__icon">🧹</span>
            <span class="dash-card__title">Ověřování kontaktů</span>
            <span class="dash-card__desc">Čistička databáze</span>
        </a>
    ...
    <!-- ── Obchodák (jen vlastní pohled) ── -->
    <?php if ($role === 'obchodak') { ?>
    <div class="dash-cards">
        <a href="<?= crm_h(crm_url('/oz/queue')) ?>" class="dash-card dash-card--primary">
            <span class="dash-card__icon">📋</span>
            <span class="dash-card__title">Příchozí leady</span>
            <span class="dash-card__desc">Přijmout nové + zaměřené řešení</span>
        </a>
        <a href="<?= crm_h(crm_url('/oz/leads')) ?>" class="dash-card">
            <span class="dash-card__icon">💼</span>
            <span class="dash-card__title">Plná pracovní plocha</span>
            <span class="dash-card__desc">Komplexní akce (smlouvy, BO, šance)</span>
        </a>
        <a href="<?= crm_h(crm_url('/oz')) ?>" class="dash-card">
            <span class="dash-card__icon">📊</span>
            <span class="dash-card__title">Moje kvóty</span>
            <span class="dash-card__desc">Přehled &amp; reklamace</span>
        </a>
        <a href="<?= crm_h(crm_url('/oz/performance')) ?>" class="dash-card">
            <span class="dash-card__icon">🏅</span>
            <span class="dash-card__title">Výkon týmu</span>
            <span class="dash-card__desc">BMSL &amp; stage milníky</span>
        </a>
    </div>
    <?php } ?>

    <!-- ── Back-office (jen vlastní pohled) ── -->
    <?php if ($role === 'backoffice') { ?>
    <div class="dash-cards">
        <a href="<?= crm_h(crm_url('/bo')) ?>" class="dash-card dash-card--primary">
            <span class="dash-card__icon">🏢</span>
            <span class="dash-card__title">Back-office</span>
            <span class="dash-card__desc">Příprava smluv, datovka, OKU</span>
        </a>
    ...
    <div class="dash-meta" style="display:flex;align-items:center;gap:0.6rem;flex-wrap:wrap;">
        <span><?= crm_h((string) ($user['email'] ?? '')) ?></span>
        <span style="opacity:0.4;">·</span>
        <a href="<?= crm_h(crm_url('/account/password')) ?>"
           style="color:var(--accent);text-decoration:none;font-size:0.78rem;">
            🔒 Změnit heslo
        </a>
    </div>

    <form method="post" action="<?= crm_h(crm_url('/logout')) ?>" class="inline" style="margin-top:1rem;">
